...
from nengo import Oja
from nengo.solvers import LstsqL2
import pandas as pd

# weight_after = pd.read_csv('Weigth.csv', header=None)
...
# # W = weight_after.applymap(str_to_float).values.flatten()
...
def plot_xy(data, num_xticks=5, num_yticks=5, xmin=None, xmax=None, ymin=None, ymax=None):
    ...
    ax4.plot(sim.trange(sample_every=0.1), data[weights][..., 10],"--")
    ax4.set_xlabel('Tiempo')
    ax4.set_ylabel('Input')
    ax4.grid(True, linestyle='-.')
    ax4.legend(fontsize='14')
    ...
    # ax4.grid(True, linestyle='-.')
    # ax4.legend(fontsize='14')
    ax4.plot(t, data[gamma][:,0],"--", label='z_estimada')
    #ax4.set_xlabel('Tiempo')
    #ax4.set_ylabel('Input')
    ax4.grid(True, linestyle='-.')
    ax4.legend(fontsize='14')
    ax5.plot(t, data[gamma][:,1],"--", label='x_estimada')
    #ax5.set_xlabel('Tiempo')
    ax5.set_ylabel('Distance')
    ax5.grid(True, linestyle='-.')
    ax5.legend(fontsize='14')
    ax6.plot(t, data[gamma][:,2],"--", label='y estimada')
    ax6.set_xlabel('Tiempo')
    #ax6.set_ylabel('Input')
    ax6.grid(True, linestyle='-.')
    ax6.legend(fontsize='14')
...
class Models (object):
    def __init__(self, frecuancy):
        ...
        with self.model:
            ...
            nengo.Connection(Gamma, error, synapse=self.synaps)
            ...
            Gamma,
            transform=0,
            #solver=LstsqL2(weights=True),  # Utiliza LstsqL2 para permitir la especificación de pesos iniciales
            #solver_info={"weights": W},
            #learning_rule_type = Oja(), , pre_synapse=self.synaps
            ...
            self.weights_p = nengo.Probe(conn,"weights", sample_every=0.1, synapse=0.01)
    ...
    def Gradientes(self):
        return self.gradiente_Kp, self.gradiente_Ki, self.gradiente_Kd, self.Gamma
    def get_model(self):
        return self.model
    def get_probe(self):
...
u_ax = m.SeñalInput()
gradiente_kp, gradiente_ki, gradiente_kd, gamma = m.Gradientes()
