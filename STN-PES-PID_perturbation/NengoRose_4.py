...
def plot_xy(data, num_xticks=5, num_yticks=5, xmin=None, xmax=None, ymin=None, ymax=None):
    ...
    ax6.plot(t, data[gradiente_dk],"--", label='gradiente kd')
    ...
    fig6, (ax4) = plt.subplots(figsize=(12, 8))
    ax4.plot(sim.trange(sample_every=0.01), data[weights][..., 10],"--")
    ax4.set_xlabel('Tiempo')
    ax4.set_ylabel('Input')
    ax4.grid(True, linestyle='-.')
    ax4.legend(fontsize='14')
...
##=========================================================================================================
    # fig6, (ax4, ax5, ax6) = plt.subplots(3, 1, figsize=(12, 8))
    # # ax4.plot(sim.trange(), data[gamma],"--")
    # # ax4.set_xlabel('Tiempo')
    # # ax4.set_ylabel('Distance[m]')
    # # ax4.grid(True, linestyle='-.')
    # # ax4.legend(fontsize='14')
    # ax4.plot(t, data[estimation_pos][:,0],"--", label='z_estimada')
    # ax4.plot(t, data[posicion][:, 0],"--", label='z')
    # #ax4.set_xlabel('Tiempo')
    # #ax4.set_ylabel('Input')
    # ax4.grid(True, linestyle='-.')
    # ax4.legend(fontsize='14')
    # ax5.plot(t, data[estimation_pos][:,1],"--", label='x_estimada')
    # ax5.plot(t, data[posicion][:,1],"--", label='x')
    # #ax5.set_xlabel('Tiempo')
    # ax5.set_ylabel('Distance')
    # ax5.grid(True, linestyle='-.')
    # ax5.legend(fontsize='14')
    # ax6.plot(t, data[estimation_pos][:,2],"--", label='y estimada')
    # ax6.plot(t, data[posicion][:,2],"--", label='y')
    # ax6.set_xlabel('Tiempo')
    # #ax6.set_ylabel('Input')
    # ax6.grid(True, linestyle='-.')
    # ax6.legend(fontsize='14')

    # plt.savefig("Identificacion.pdf")

    fig7, (ax4, ax5, ax6) = plt.subplots(3, 1, figsize=(12, 8))
    # ax4.plot(sim.trange(), data[gamma],"--")
    # ax4.set_xlabel('Tiempo')
    # ax4.set_ylabel('Distance[m]')
    # ax4.grid(True, linestyle='-.')
    # ax4.legend(fontsize='14')
    ax4.plot(t, data[Gamma][:,0],"--", label='z_Derivada_output_input')
    ax4.plot(t, data[estimation_deriv][:, 0],"--", label='Derivada_output_input_est')
    ax4.set_ylim(-2, 2) 
    #ax4.set_xlabel('Tiempo')
    #ax4.set_ylabel('Input')
    ax4.grid(True, linestyle='-.')
    ax4.legend(fontsize='14')
    ax5.plot(t, data[Gamma][:,1],"--", label='x_Derivada_output_input')
    ax5.plot(t, data[estimation_deriv][:,1],"--", label='Derivada_output_input_est')
    ax5.set_ylim(-2, 2) 
    #ax5.set_xlabel('Tiempo')
    ax5.set_ylabel('Distance')
    ax5.grid(True, linestyle='-.')
    ax5.legend(fontsize='14')
    ax6.plot(t, data[Gamma][:,2],"--", label='y_Derivada_output_input')
    ax6.plot(t, data[estimation_deriv][:,2],"--", label='Derivada_output_input_est')
    ax6.set_ylim(-2, 2) 
    ax6.set_xlabel('Tiempo')
    #ax6.set_ylabel('Input')
    ax6.grid(True, linestyle='-.')
    ...
    plt.savefig("Derivada_estimacion.pdf")
###==============================================================================================================
...
###==============================================================================================================
    ...
    # with open('Estimado.csv', 'w', newline='') as file:
    #     writer = csv.writer(file)
    #     writer.writerow(data[estimation_pos])
    with open('Derivada.csv', 'w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(data[estimation_deriv])
...
class ExternalInput(nengo.Node):
    Kp = [165, 85, 85]  # Declare Kp as a class variable
    Kd = [20, 15, 15] 
    Ki = [150, 215, 215]
    def __init__(self, name):
        ...
        self.mu_Ki = 2e-5
        self. mu_Kd = 6e-4
        self.dest_sub = rospy.Subscriber(f'/mocap_node/tello_10/Odom', Odometry, self.callback, queue_size=1)      
    ...
    def tick(self,t, valores):
        # if self.pose[2] >1.8 or abs(self.pose[0])>1.3 or abs(self.pose[1])>1.3:
        #     MyTello.send_rc_control(0, 0 ,0, 0)#############################################################
        #     MyTello.land()
        #     print("")
...
class Models (object):
    def __init__(self, frecuancy):
        ...
        with self.model:
            ...
            def reference_signals_xy_func(t):
                if t > 5 and t<12:
                    T = t-5
                    return[self.SetpointFinal[1]/(1 + math.exp(-(3*T-5))), 0]
                if t >= 12.0:
                    T = t-12
            ...
            ###########################################################################################################
            ###########################################################################################################
            # inhib_node2 = nengo.Node(inhib2)
            # Estimacion_pos = nengo.Ensemble(
            #         n_neurons=self.J,
            #         dimensions=self.O,
            #         intercepts=nengo.dists.Uniform(0.0, 1),
            #     )####Estima la posicion de z,x,y con respecto a una entrada
            Gamma = nengo.Ensemble(
                    n_neurons=self.J,
                    dimensions=self.O,
                    intercepts=nengo.dists.Uniform(0.0, 1),
                )####Gamma calcula el cambio y vs u
            #============================================================================================================
            DeltaY_estimation = nengo.Ensemble(
                    n_neurons=self.J,
                    dimensions=self.O,
                    intercepts=nengo.dists.Uniform(0.0, 1),
                ) ####DeltaY_estimation debe estimar el cambio de Y con respecto a una entrada U
            DeltaY = nengo.Ensemble(n_neurons=self.J, dimensions = self.O, radius = 2)
            nengo.Connection(Input_encoder,DeltaY,synapse=0.05,transform= self.Frecuncy) #################### F = 30 Hz
            nengo.Connection(Input_encoder,DeltaY,synapse=0.1,transform= -self.Frecuncy) ##############################:
            DeltaU = nengo.Ensemble(n_neurons=self.J, dimensions = self.I, radius = 2)
            nengo.Connection(controlEns,DeltaU,synapse=0.05,transform= self.Frecuncy) #################### F = 30 Hz
            nengo.Connection(controlEns,DeltaU,synapse=0.1,transform= -self.Frecuncy) ##############################:
            #============================================================================================================
            # error = nengo.Ensemble(n_neurons=self.J, dimensions=self.O)
            # nengo.Connection(Input_encoder, error, transform=-1, synapse=self.synaps)########
            # nengo.Connection(Estimacion_pos, error, synapse=self.synaps)
            # nengo.Connection(inhib_node2, error.neurons, transform=[[-10]] * error.n_neurons)
            def division(t,x):
                d_y = x[:3] 
                d_x = x[3:]
                if np.any(d_x != 0):
            ...
            nengo.Connection(DeltaY, Gamma[:3], synapse=self.synaps*10)
            nengo.Connection(DeltaU, Gamma[3:], synapse=self.synaps*10)
            # conn = nengo.Connection(
            # controlEns,
            # Estimacion_pos,
            # transform=0,
            # learning_rule_type=PES(learning_rate=5e-4),
            #     )
            #==========================================================================================================
            ...
            error2 = nengo.Ensemble(n_neurons=self.J, dimensions=self.O)
            nengo.Connection(Gamma, error2, transform=-1, synapse=self.synaps)########
            nengo.Connection(DeltaY_estimation, error2, synapse=self.synaps)
            nengo.Connection(error2, conn1.learning_rule)
            #=========================================================================================================
            #nengo.Connection(error, conn.learning_rule)
            nengo.Connection(DeltaY_estimation, Grad_kp[:3], synapse=self.synaps)
            nengo.Connection(DeltaY_estimation, Grad_ki[:3], synapse=self.synaps)
            nengo.Connection(DeltaY_estimation, Grad_kd[:3], synapse=self.synaps)
            ...
            nengo.Connection(Grad_kp, external_input[[3, 4, 5]], synapse=0.1)### GRAD_kp= e**2 * dy/du
            nengo.Connection(Grad_ki, external_input[[6, 7, 8]], synapse=0.1)### GRAD_ki= e * dy/du * int_(e)
            nengo.Connection(Grad_kd, external_input[9:], synapse=0.1)########## GRAD_kd= e * dy/du * de/dkp
            ...
            self.gradiente_Kp = nengo.Probe(Grad_kp, synapse=self.synaps)
            self.gradiente_Ki = nengo.Probe(Grad_ki, synapse=self.synaps)
            self.gradiente_Kd = nengo.Probe(Grad_kd, synapse=self.synaps)
            self.weights_p = nengo.Probe(conn1,"weights", sample_every=0.01, synapse=0.01)
            #self.Estim_pos = nengo.Probe(Estimacion_pos, synapse=self.synaps)
            self.Gamma = nengo.Probe(Gamma, synapse=self.synaps)
            self.Estim_deriv = nengo.Probe(DeltaY_estimation, synapse=self.synaps)
    ...
    def get_model(self):
        return self.model
    def Gradientes(self):
        return self.gradiente_Kp, self.gradiente_Ki, self.gradiente_Kd, self.Estim_deriv, self.Gamma
    def get_probe(self):
        return self.inputs, self.ref_encod, self.ref_encodXY, self.error_probe, self.weights_p
...
#################
posicion, ref, refXY, error, weights = m.get_probe()
u_ax = m.SeñalInput()
gradiente_kp, gradiente_ki, gradiente_dk, estimation_deriv, Gamma = m.Gradientes()
