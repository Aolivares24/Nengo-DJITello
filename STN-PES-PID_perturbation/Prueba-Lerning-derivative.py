...
class ExternalInput(nengo.Node):
    Kp = [160, 62, 62]  # Declare Kp as a class variable
    Kd = [20, 15, 15] 
    Ki = [150, 200, 200]
    ...
    def __init__(self, frecuancy):
        ...
        with self.model:
            ...
            def reference_signals_xy_func(t):
                # if t > 5 and t<12:
                #     T = t-5
                #     return[self.SetpointFinal[1]/(1 + math.exp(-(2*T-5))), 0]
                if t >= 5:
                    T = t-5
                    return [self.SetpointFinal[1]*math.sin(T), 
                            0]
                else:
            ...
            nengo.Connection(DeltaY, Gamma[:3], synapse=self.synaps)
            nengo.Connection(DeltaU, Gamma[3:], synapse=self.synaps)
